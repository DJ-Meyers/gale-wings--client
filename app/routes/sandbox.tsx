...
const PlayerPokemonPanel = () => {
  ...
  const setPlayer = useCalcStore((s) => s.setPlayer)
...
const SandboxLayout = () => {
  const playerSpecies = useCalcStore((s) => s.player.species)
  const calcOrder = useCalcStore((s) => s.calcOrder)
  const calcs = useCalcStore((s) => s.calcs)

  const { offensive, defensive, speed } = useMemo(() => {
    const offensive: Calc[] = []
    const defensive: Calc[] = []
    const speed: Calc[] = []
